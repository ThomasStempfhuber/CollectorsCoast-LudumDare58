...
func produce(delta):
	production = production + fruit_count * RATE * delta
	if production >= 100:
		harvest()

func harvest():
	if Global.fruit < Global.storage_limit:
		Global.fruit = Global.fruit + 1
		Global.fruit_collected = Global.fruit_collected + 1
		$ResLabel.show_label()
		production = 0
	else:
		production = 100
...
func place():
	...
	placed = true
	Global.fruitfarms_built = Global.fruitfarms_built + 1
	fruit_count = res_collisions
	$FarmArea.free()
	$ProdBar.show()
...
func abort():
	Global.placing = false
	Global.stone = min(Global.stone + Global.FRF_STONE_COST , Global.storage_limit)
	Global.wood  = min(Global.wood  + Global.FRF_WOOD_COST , Global.storage_limit)
	self.queue_free()
